...
def otp(otp,rollno,name,group,password,email):
    if request.method=='POST':
        ...
        if otp==uotp:
            ...
            flash('wrong otp')
            return render_template('otp=otp',rollno=rollno,name=name,group=group,password=password,email=email)
@app.route('/noteshome')
def noteshome():
    if session.get('user'):
...
def addnote():
    if session.get('user'):
        if request.method=='POST':
            ...
            mysql.connection.commit()
            cursor.close()
            flash(f'{title}add successfully')
...
def filedelete(fid):
    cursor=mysql.connection.cursor()
    cusor.execute('delete form files where fid=%s',[fid])
    mysql.connection.commit()
    cursor.close()
...
def forget():
    if request.method=='POST':
        ...
        if (rollno,) in data:
            ...
            flash('Rest link sent to your mail')
            return redirect(url_for('login'))
        else:
